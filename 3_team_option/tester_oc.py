...
class Tester:
    ...
    def test_play(self, current_weights):
        ...
        if result['individual_return'] >= self.max_returns:
            ...
            save_name = '/best_model_team_return/'
...
def main():
    ...
    _, agents_q_omega, _, _ = oc_model([padded_obs, global_state], mask)  # (1,n,option_dim)
    option = get_option(agents_q_omega, mask, policy_over_options)  # (1,), ndarray

    # Load model
    load_dir = Path(__file__).parent / 'trial/models'
    # load_name = '/best_model|_team_return/'
    load_name = '/oc_model_1345000/'
