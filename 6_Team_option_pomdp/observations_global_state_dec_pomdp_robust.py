...
from utils import compute_global_blue_maps, compute_global_red_maps

"""
    Copied MARL_Transformer_Actor_Critic/5_Ind_option_pomdp
    (Originally Copied from QMIX-2 (QMIX Another Implementation))
"""
...
def get_global_observation(env):
    red_normalized_force, red_efficiency = compute_global_red_maps(env)  # (global_g,global_g)
    blue_normalized_force, blue_efficiency = compute_global_blue_maps(env)  # (global_g,global_g)
    sin_normalized, cos_normalized = compute_global_red_pos_maps(env)  # (global_g,global_g)

    red_normalized_force = add_channel_dim(red_normalized_force)  # (global_g,global_g,1)
    red_efficiency = add_channel_dim(red_efficiency)  # (global_g,global_g,1)

    blue_normalized_force = add_channel_dim(blue_normalized_force)  # (global_g,global_g,1)
    blue_efficiency = add_channel_dim(blue_efficiency)  # (global_g,global_g,1)

    sin_normalized = add_channel_dim(sin_normalized)  # (global_g,global_g,1)
    cos_normalized = add_channel_dim(cos_normalized)  # (global_g,global_g,1)

    observation = np.concatenate([
        red_normalized_force, red_efficiency,
        blue_normalized_force, blue_efficiency,
        sin_normalized, cos_normalized
    ], axis=-1)  # (global_g,global_g,6)

    return observation  # (global_g,global_g,6)
