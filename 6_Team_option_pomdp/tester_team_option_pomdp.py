"""
Copied from /Applying_Option_Critic_to_MARL_Transformer/
            5_Ind_option_pomdp/tester_indiv_option_pomdp_scenario_test.py,
then modified termination_head
"""
import os.path
import random
import json
...
class Tester:
    def __init__(self):
        ...
        self.oc_model = Team_OptionCritic_Model(config=self.env.config)
    ...
    def reset_states(self, observations, global_observation):
        ...
        # (b,option_dim),(b,n,option_dim),(b,n,option_dim,action_dim), b=1

        self.option = (
            get_option(self.agents_q_omega, self.mask, self.policy_over_options))  # (1,)
    ...
    def test_play(self, current_weights):
        ...
        for i_episode in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                next_terminations = self.oc_model.termination_head.sample_termination(
                    next_termination_probs, self.option)  # (1,1), bool

                if next_terminations[0, 0]:  # bool
                    self.option = get_option(
                        next_agents_q_omega, next_mask, self.policy_over_options
                    )  # (1,), ndarray
...
def main(is_debug, is_test):
    ...
    oc_model = Team_OptionCritic_Model(config)
    ...
    option = get_option(agents_q_omega, mask, policy_over_options)  # (1,), ndarray

    # Load model
    if is_test:
        load_dir = Path(__file__).parent / 'trial/models'
        # load_name = '/oc_model_2155000/'
        load_name = '/best_model_individual_return/'
