...
from models_team_options_pomdp import EpsGreedyOverOptions, Team_OptionCritic_Model
from utils_transformer_mtc_dec_pomdp import (make_mask, make_po_attention_mask, make_padded_obs,
                                             make_padded_pos)
from utils_team_opion import get_option

from worker_team_option import Worker
from tester_team_option_pomdp import Tester
...
def learn(num_workers=8, is_debug=False):
    ...
    oc_model = Team_OptionCritic_Model(config=env.config)
    target_oc_model = Team_OptionCritic_Model(config=env.config)
    ...
    while update_cycles <= env.config.num_update_cycles:
        ...
        betas_dash, target_q_omegas, _, _ = \
            target_oc_model([[next_obss, next_poss], next_global_obss],
                            next_masks, next_attention_masks)
