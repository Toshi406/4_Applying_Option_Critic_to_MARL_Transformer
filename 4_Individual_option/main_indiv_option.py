...
from models_indiv_options import EpsGreedyOverOptions, Individual_OptionCritic_Model
from utils_transformer import make_mask, make_padded_obs
from utils_ndiv_opion import get_options, get_agent_option

from worker_indiv_option import Worker
from tester_indiv_option import Tester
...
def learn(num_workers=8, is_debug=False):
    ...
    while update_cycles <= env.config.num_update_cycles:
        ...
        """
        # Make lists
        (states, next_states, actions, rs, dones, masks, next_masks, options,
         global_states, next_global_states, team_rs, team_dones) = \
            [], [], [], [], [], [], [], [], [], [], [], []

        for i in range(num_workers):
            states.append(trajectories[i]["s"])  # append (b,n,g,g,ch*n_frames), b=1
        ...
        # lists -> np.array
        states = np.array(states, dtype=np.float32)  # (w,b,n,g,g,ch*n_frames), b=1
