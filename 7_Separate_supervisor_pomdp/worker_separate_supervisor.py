"""
Copied from Applying_Option_Critic_to_MARL_Transformer/6_Team_option_pomdp/worker_team_option.py,
then modified termination
"""
import ray
import numpy as np
...
class Worker:
    ...
    def rollout_and_collect_trajectory(self, weights):
        ...
        0. Global policyの重みをコピー 
        """
        self.supervisor.set_weights(weights=weights[0])
        self.po_agents.set_weights(weights=weights[1])
    ...
    def _rollout(self):
        ...
        for i in range(self.batch_size):
            ...
            # trajectory["options"].append(self.option)  # append (1,n)
        ...
        trajectory["global_s2"] = (
            np.concatenate(trajectory["global_s2"], axis=0).astype(np.float32))
        # (b, g, g, global_ch * global_n_frames)
