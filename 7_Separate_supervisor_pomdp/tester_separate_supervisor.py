"""
Copied from 6_Team_option_pomdp/tester_team_option_pomdp.py.  Then modified.
"""
import os.path
import random
import json
...
class Tester:
    def __init__(self):
        ...
        self.supervisor = SupervisorModel(config=self.env.config)
        self.po_agents = POAgentsModel(config=self.env.config)
    ...
    def reset_states(self, observations, global_observation):
        ...
        # build supervisor, and get the option
        self.termination_probs, self.Q_Omega = self.supervisor(self.global_state)
        # (b,option_dim),(b,option_dim), b=1

        self.option = get_option(self.Q_Omega, self.policy_over_options)  # (1,)

        # build po_agents
        self.agents_policy_logit, _ = self.po_agents(
            [self.padded_obss, self.padded_poss],
            self.mask,
            self.attention_mask,
            training=False)  # (b,n,option_dim,action_dim)
    ...
    def test_play(self, current_weights):
        # 重みを更新
        self.supervisor.set_weights(weights=current_weights[0])
        self.po_agents.set_weights(weights=current_weights[1])
        ...
        for i_episode in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                acts = self.po_agents.agents_policy_head.sample_actions(
                ...
                # Check termination @ supervisor
                next_termination_probs, next_Q_Omega = self.supervisor(next_global_state)
                # (b,option_dim),(b,option_dim)

                next_terminations = self.supervisor.termination_head.sample_termination(
                    next_termination_probs, self.option)  # (1,1), bool

                if next_terminations[0, 0]:  # bool
                    self.option = (
                        get_option(next_Q_Omega, self.policy_over_options))  # (1,), ndarray

                    option_switches += 1

                # update policies @ po_agents
                next_agents_policy_logit, scores = (
                    self.po_agents(
                        [next_padded_obss, next_padded_poss],
                        next_mask,
                        next_attention_mask,
                        training=False))
                # (b,n,option_dim,action_dim), [(b,num_heads,n,n],(b,num_heads,n,n]]
                ...
                if dones['all_dones']:
                    ...
                    self.termination_probs = next_termination_probs  # (1,4)
                    self.Q_Omega = next_Q_Omega  # (1,4)
        ...
        if result['individual_return'] >= self.max_returns:
            ...
            save_name = '/best_supervisor/'
            self.supervisor.save_weights(str(save_dir) + save_name)
            save_name = '/best_po_agents/'
            self.po_agents.save_weights(str(save_dir) + save_name)
...
def main(is_debug, is_test):
    ...
    po_agents = POAgentsModel(config)
    ...
    global_state_shape = (global_grid_size, global_grid_size, global_ch * global_n_frames)
    global_state = \
    ...
    # Supervisor
    _, Q_Omega = supervisor(global_state)  # (1,option_dim)
    option = get_option(Q_Omega, policy_over_options)  # (1,), ndarray

    # po_agents
    _, _ = po_agents([padded_obs, padded_pos], mask, attention_mask, training=False)

    # Load model
    if is_test:
        load_dir = Path(__file__).parent / 'trial/models'
        # load_name = '/oc_model_2155000/'
        load_name = '/best_supervisor/'
        supervisor.load_weights(str(load_dir) + load_name)

        load_name = '/best_po_agents/'
        po_agents.load_weights(str(load_dir) + load_name)
...
if __name__ == '__main__':
    ...
    is_test = True # True for test after training

    main(is_debug, is_test)
