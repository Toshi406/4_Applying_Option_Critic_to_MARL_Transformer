"""
Copied from Applying_Option_Critic_to_MARL_Transformer/4_Individual_option/main_indv_option.py,
...
from worker_indiv_option import Worker
from tester_indiv_option_pomdp import Tester
...
def learn(num_workers=8, is_debug=False):
    ...
    # Build models
    oc_model([[padded_obs, padded_pos], global_state], mask, attention_mask)  # build
    target_oc_model([[padded_obs, padded_pos], global_state], mask, attention_mask)  # build
    ...
    while update_cycles <= env.config.num_update_cycles:
        ...
        obss = obss.reshape([batch_size, max_num_agents, 2 * fov + 1, 2 * fov + 1, ch * n_frames])
        ...
        attention_masks = attention_masks.reshape(([batch_size, max_num_agents, max_num_agents]))
        ...
        options_onehot = \
            tf.one_hot(options, depth=env.config.option_dim, axis=-1)  # (b,n,option_dim)
        ...
        betas_dash, target_q_omegas, _, _ = \
            target_oc_model([[next_obss, next_poss], next_global_obss], next_masks,
                            next_attention_masks)
        # (b,n,option_dim), (b,n,option_dim)

        betas_dash = tf.reduce_sum(betas_dash * options_onehot, axis=-1)  # (b,n)

        target_Q_Omegas = target_q_omegas * options_onehot  # (b,n,option_dim)
        target_Q_Omegas = tf.reduce_sum(target_Q_Omegas, axis=-1)  # (b,n)

        target_Q_Omega_max = np.max(target_q_omegas.numpy(), axis=-1)  # (b,n)

        gt = rs + env.config.gamma * (1. - float_dones) * \
             ((1. - betas_dash) * target_Q_Omegas + betas_dash * target_Q_Omega_max)  # (b,n)

        with tf.GradientTape() as tape:
            # termination_probs: (b,n,option_dim)
            ...
            termination_probs, agents_q_omega, agents_policy_logit, _ = \
                oc_model([[obss, poss], global_obss], masks, attention_masks)

            ### value_loss, {Q^i(s^i,ω)}, i=1:n,

            Q_Omegas = agents_q_omega * options_onehot  # (b,n,option_dim)
            Q_Omegas = tf.reduce_sum(Q_Omegas, axis=-1)  # (b,n)

            Q_Omegas_max = np.max(agents_q_omega.numpy(), axis=-1)  # (b,n)
            ...
            policy_probs = policy_probs * options_onehot_extended  # (b,n,option_dim,action_dim)
            policy_probs = tf.reduce_sum(policy_probs, axis=2)  # (b,n,action_dim)

            log_policy = log_policy * options_onehot_extended  # (b,n,option_dim,action_dim)
            ...
            beta = termination_probs * options_onehot  # (b,n,option_dim)
            beta = tf.reduce_sum(beta, axis=-1)  # (b,n)

            term_losses = beta * tf.stop_gradient(advantage)  # (b,n)
            term_losses = tf.reduce_sum(term_losses, axis=-1) / num_alive_agents  # (b,)

            termination_loss = tf.reduce_mean(term_losses + termination_reg)
