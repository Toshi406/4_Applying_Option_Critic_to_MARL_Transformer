...
class AgentPolicyHead(tf.keras.models.Model):
    ...
    def policy_pdf(self, policy_logits, mask):
        ...
        log_probs = tf.nn.log_softmax(policy_logits, axis=-1)  # (None,n,option_dim,action_dim)
    ...
    def sample_actions(self, policy_logits, options, mask):
        ...
        options_onehot = tf.one_hot(options, depth=self.config.option_dim)  # (b,option_dim)
        options_onehot_extended = tf.expand_dims(options_onehot, axis=1)  # (b,1,option_dim)
        options_onehot_extended = \
            tf.expand_dims(options_onehot_extended, axis=-1)  # (b,1,option_dim,1)
...
def main():
    ...
    """ Sample actions """
    options = np.random.randint(low=0, high=config.option_dim, size=(config.batch_size,))  # (b,)
